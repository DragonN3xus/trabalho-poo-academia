...
public class MenuFuncionario implements Menu {
	...
	private void cadastrarAluno() {
		...
		for (Plano plano : Banco.planos) {
		    String modalidadesStr = plano.getModalidades().stream()
		        .map(Enum::name)
		        .collect(Collectors.joining(", "));

		    System.out.printf(
		        "%-5d %-15s %-40s R$%-10.2f%n",
		        plano.getId(),
		        plano.getPeriodo().name(),
		        modalidadesStr,
		        plano.getValor()
		    );
		}
	...
	private void cadastrarPlano() {
		...
		do {
		    ...
		    switch (opcaoModalidade) {
		        case 1: modalidade = Modalidade.MUSCULACAO; break;
		        case 2: modalidade = Modalidade.FUNCIONAL; break;
		        case 3: modalidade = Modalidade.PILATES; break;
		        case 4: modalidade = Modalidade.JIUJITSU; break;
		        case 5: modalidade = Modalidade.BOXE; break;
		        default:
		            System.out.println("Opção inválida!");
		            continue;
		    }
		...
		int id = Banco.planos.stream()
		        .mapToInt(Plano::getId)
		        .max()
		        .orElse(0) + 1;
